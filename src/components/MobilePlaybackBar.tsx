...
export function MobilePlaybackBar({
  ...
  const bar = (
    <div
      className="
        ...
        pb-[max(0.5rem,min(env(safe-area-inset-bottom),40px))]
        lg:hidden
      "
    >
  ...
  // Portal to document.body so no ancestor's `transform`, `contain`, or
  // `filter` can turn into an unexpected containing block and push our
  // `fixed` bar off the viewport bottom. (.panel-hover-lift uses
  // `contain: layout paint style`, which elsewhere in the tree WOULD
